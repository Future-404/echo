...
1. **Tool Usage**: Use \`manage_quest_state\` when the conversation dictates a shift in overarching goals.
   - **action**: "CREATE" (new objective), "UPDATE" (progress change), "RESOLVE" (objective achieved), "FAIL" (objective failed/abandoned).
...
2. **Objective Logic**:
   - **MAIN Objective**: Only ONE active MAIN objective is allowed. Do not CREATE a new one until the current one is RESOLVED or FAILED.
...
3. **Pacing & Friction**:
   ...
   - Use FAIL when an objective becomes permanently impossible or the narrative has moved past it.
