...
export const executeQuestSkill = (args: any, store: any): SkillExecuteResult => {
  const { action, quest_id, quest_type, title, description, progress_delta, ui_toast } = args;
  const state = store.getState();

  if (action === 'CREATE') {
    const exists = state.missions.some((m: Mission) => m.id === quest_id);
    if (exists) {
      // id 已存在，降级为 UPDATE
      ...
      store.setState((s: any) => ({ missions: [...s.missions, newQuest] }));
    }
  }
