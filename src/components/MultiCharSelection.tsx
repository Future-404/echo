  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-7xl">
        <header className="mb-10 text-center">
          <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="flex items-center justify-center gap-3 mb-3">
            ...
            <h2 className="text-[12px] tracking-[0.8em] text-echo-text-muted uppercase font-medium">Multi-Character Mode</h2>
          </motion.div>
            ...
            return (
              <motion.div
                ...
                className={`snap-center flex-shrink-0 w-56 md:w-64 h-[380px] md:h-[420px] backdrop-blur-2xl border-0.5 rounded-[3rem] cursor-pointer flex flex-col transition-all shadow-xl relative select-none
                  ${isA ? 'bg-blue-500/10 border-blue-400/40' : isB ? 'bg-purple-500/10 border-purple-400/40' : 'bg-white/80 dark:bg-white/5 border-echo-border-md hover:shadow-2xl'}`}
              >
                {/* 角色标签 */}
                <AnimatePresence>
                  {selected && (
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 pointer-events-none">
                  <div className="w-24 h-24 mb-8 relative">
                    <div className={`absolute inset-0 rounded-full border-0.5 scale-125 ${selected ? 'border-current animate-pulse' : 'border-echo-border'}`} />
                ...
                <div className="px-8 pb-10 pointer-events-none">
                  <p className="text-[9px] text-echo-text-subtle text-center leading-relaxed tracking-widest uppercase italic line-clamp-2">
                    {char.description}
                  </p>
                </div>
              </motion.div>
      ...
      <div className="fixed bottom-8 left-0 right-0 flex items-center justify-center gap-4 z-20">
        <button onClick={handleBack} className="px-6 py-3 rounded-full text-[10px] tracking-widest uppercase text-gray-400 hover:text-echo-text-base transition-colors">
          返回
        </button>
        <AnimatePresence>
          {canStart && (
