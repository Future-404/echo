    ...
    reader.onload = (event) => {
      ...
      if (file.name.endsWith('.json')) {
        try {
          ...
          // 兼容多种格式：直接是 entries 数组，或者是包含 entries 的对象
        ...
        <div className="flex justify-between items-end px-2">
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              className="p-2 bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-full text-gray-400 hover:text-blue-500 transition-all shadow-sm"
              title="导入 JSON/TXT"
            >
              <Upload size={14} />
            </button>
            <button 
              ...
              className="p-2 bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-full text-gray-400 hover:text-black dark:hover:text-white transition-all shadow-sm"
              title="新建世界书"
            >
              <Plus size={14} />
            </button>
          ...
          {library.length === 0 ? (
            ...
            library.map(book => (
              <div 
                ...
                className="group p-4 bg-white/50 dark:bg-white/5 border border-gray-100 dark:border-white/5 rounded-2xl cursor-pointer hover:border-blue-400/30 transition-all flex items-center justify-between"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-sm font-serif font-bold text-gray-700 dark:text-gray-200">{book.name}</h4>
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="p-6 h-full flex flex-col space-y-6">
      <div className="flex items-center justify-between px-2">
        ...
        <button onClick={handleAddEntry} className="p-2 bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-full text-gray-400 hover:text-black dark:hover:text-white transition-all">
          <Plus size={14} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-2 no-scrollbar pb-10">
        {selectedBook?.entries.map((entry) => (
          <div key={entry.id} className="bg-white/50 dark:bg-white/5 border-0.5 border-gray-100 dark:border-white/5 rounded-2xl overflow-hidden transition-all">
            <div 
            ...
            <AnimatePresence>
              {expandedEntryId === entry.id && (
                <motion.div 
                  ...
                  className="px-4 pb-4 border-t border-gray-100 dark:border-white/5 space-y-4 pt-4"
                >
                  <div className="space-y-1">
                    ...
                    <input 
                      ...
                      className="w-full bg-white dark:bg-black/20 border-0.5 border-gray-100 dark:border-white/10 rounded-xl px-3 py-2 text-[10px] font-mono focus:outline-none focus:border-blue-400/50 transition-colors"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea 
                      ...
                      className="w-full bg-white dark:bg-black/20 border-0.5 border-gray-100 dark:border-white/10 rounded-xl px-3 py-2 text-[11px] font-serif min-h-[120px] focus:outline-none focus:border-blue-400/50 transition-colors resize-none no-scrollbar"
                    />
                  </div>
