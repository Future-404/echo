...
const IdentityLink: React.FC<IdentityLinkProps> = ({ onClick }) => {
  ...
  return (
    <div className="space-y-4">
      <label className="text-[9px] tracking-widest text-gray-300 dark:text-gray-600 uppercase italic px-4 underline decoration-gray-100 dark:decoration-gray-800 underline-offset-8">Identifier</label>
      <div 
        onClick={onClick}
      ...
      >
        ...
        <div className="flex-1">
          <h3 className="text-sm font-serif text-gray-600 dark:text-gray-300 tracking-widest">{selectedCharacter.name}</h3>
          <p className="text-[8px] text-gray-400 dark:text-gray-500 uppercase mt-1">Active Sync</p>
        </div>
        <ChevronRight size={16} strokeWidth={1} className="text-gray-300 dark:text-gray-700" />
      </div>
    </div>
  )
}
