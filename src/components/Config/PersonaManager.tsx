      ...
      if (file.name.endsWith('.json')) {
        try {
          ...
          items.forEach(item => {
            if (item.name) {
              addPersona({
                id: `persona-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
                name: item.name,
                ...
                >
                  ...
                  <div className="space-y-4 pt-4 border-t border-gray-100 dark:border-white/5">
                    <div className="flex justify-between items-center px-2">
                      ...
                      <button onClick={() => setIsAddingPrivate(!isAddingPrivate)} className="p-1 bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-full text-gray-400 hover:text-black dark:hover:text-white transition-all">
                        <Plus size={12} />
                      </button>
                    </div>
                    ...
                    {isAddingPrivate && (
                      <div className="bg-gray-50 dark:bg-white/5 rounded-2xl p-4 space-y-3">
                        <input placeholder="Label (Optional)" value={newPrivateKeys} onChange={e => setNewPrivateKeys(e.target.value)} className="w-full bg-transparent border-b border-gray-200 dark:border-gray-800 text-[10px] py-1 focus:outline-none text-gray-600 dark:text-gray-300" />
                        <textarea placeholder="Your private user lore..." value={newPrivateContent} onChange={e => setNewPrivateContent(e.target.value)} className="w-full bg-transparent text-[11px] font-serif min-h-[80px] resize-none focus:outline-none text-gray-600 dark:text-gray-300" />
                    ...
                    <div className="space-y-2">
                      {(p.worldBook || []).map((entry) => (
                        <div key={entry.id} className="bg-white/50 dark:bg-white/5 border-0.5 border-gray-100 dark:border-white/5 rounded-2xl overflow-hidden transition-all group/item">
                          ...
                          {expandedPrivateId === entry.id && (
                            <div className="px-3 pb-3 pt-1 border-t border-gray-100 dark:border-white/5 space-y-2">
                              <textarea 
                                value={entry.content} 
                                onChange={(e) => updatePersonaWorldBookEntry(p.id, entry.id, { content: e.target.value })}
                                className="w-full bg-white dark:bg-black/20 border-0.5 border-gray-100 dark:border-white/10 rounded-xl px-2 py-1 text-[11px] font-serif min-h-[60px] focus:outline-none focus:border-blue-400/50 transition-colors resize-none"
                              />
                            </div>
                          )}
                        </div>
