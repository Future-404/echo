    ...
    >
      <div className="flex justify-between items-center px-4">
        <div className="flex flex-col">
            ...
            <span className="text-[7px] text-gray-400 dark:text-gray-600 uppercase tracking-[0.2em] mt-0.5">Directives</span>
        </div>
        ...
        {directives.map(d => (
          ...
          >
            ...
            <div onClick={() => onEdit(d.id)} className="flex-1 cursor-pointer">
              <h4 className={`text-sm font-serif tracking-wide transition-colors ${d.enabled ? 'text-gray-600 dark:text-gray-200' : 'text-gray-400 dark:text-gray-700 line-through'}`}>
                {d.title}
              </h4>
              <p className="text-[8px] text-gray-400 dark:text-gray-600 uppercase mt-1 truncate max-w-[150px] tracking-widest">
                {d.content || '未编写内容'}
              </p>
            </div>
            <button 
              onClick={() => updateDirective(d.id, { enabled: !d.enabled })} 
              className="text-gray-400 dark:text-gray-600 hover:text-gray-500 transition-colors"
            >
      ...
      <p className="text-[8px] text-gray-400 dark:text-gray-700 text-center uppercase tracking-[0.2em] opacity-50 font-sans">
        按住并拖拽调整优先级 // 顶部优先注入
      </p>
    </motion.div>
  )
