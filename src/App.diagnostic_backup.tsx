...
const App: React.FC = () => {
  ...
  const { isLoading, setIsLoading, currentView, syncImagesFromDb, _hasHydrated, isHistoryExpanded, multiCharMode } = useAppStore()
  
  // 1. 初始化应用生命周期
  React.useEffect(() => {
    console.log('[App] Lifecycle useEffect running, _hasHydrated:', _hasHydrated);
    if (_hasHydrated) {
      syncImagesFromDb().then(() => {
        console.log('[App] Images synced');
        setIsLoading(false)
  ...
  console.log('[App] Preparing to return JSX, isLoading:', isLoading);
