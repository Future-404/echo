...
# ⚠️ 记录返回的 id，填入 wrangler.toml 的 kv_namespaces.id

# 创建 D1（存储图片）
wrangler d1 create echo-images
# ⚠️ 记录返回的 database_id，填入 wrangler.toml 的 d1_databases.database_id

# 初始化数据库表
wrangler d1 execute echo-images --file=schema.sql --remote

# 设置访问密码（这就是进入网站的密码）
openssl rand -hex 32   # 生成随机密码，复制输出
wrangler secret put AUTH_TOKEN  # 粘贴上面的字符串

# 首次手动部署 Worker
wrangler deploy
# ⚠️ 记录返回的 Worker URL：https://echo-storage.xxx.workers.dev
```

将修改后的 `wrangler.toml` commit 并 push 到你的 Fork。

#### 第三步：在 GitHub 填写 Secrets

进入你的 Fork → Settings → Secrets and variables → Actions → New repository secret：

| Secret 名称 | 值 |
|---|---|
| `CLOUDFLARE_API_TOKEN` | Cloudflare → My Profile → API Tokens → 创建（模板选 Edit Cloudflare Workers） |
| `VITE_API_URL` | 上一步的 Worker URL，如 `https://echo-storage.xxx.workers.dev` |
| `CF_PAGES_PROJECT_NAME` | 你在 Cloudflare Pages 创建的项目名，如 `echo` |

#### 第四步：在 Cloudflare Pages 创建项目（只做一次）

Cloudflare Dashboard → Pages → Create a project → Connect to Git → 选择你的 Fork，构建命令留空（由 GitHub Actions 负责）。

#### 完成！

之后每次 push 到 `main`：
- 修改了 `echo-storage/cloudflare/` → 自动重新部署 Worker
- 修改了前端代码 → 自动 build 并部署到 Pages

访问你的 Pages 域名，输入 `AUTH_TOKEN` 即可进入。
