...
前端和后端均部署到 Cloudflare Workers，全程免费额度内免费。数据默认存本地 IndexedDB，可选开启 R2 云端备份。
...
│   │   ├── index.ts             # 认证 + 适配器工厂
│   │   └── adapters/
│   │       ├── local.ts        # IndexedDB 本地适配器（主存储）
│   │       ├── remote.ts       # 远程 HTTP 适配器
│   │       └── hybrid.ts       # 本地优先混合适配器（本地 + R2 备份）
│   ├── components/
...
Echo 采用**本地优先**策略：所有数据优先读写本地 IndexedDB，后端仅作可选的云端备份。

```
读取：IndexedDB（本地）
写入：IndexedDB（本地）→ 成功后异步备份到 R2（如已配置）
```
...
| 场景 | 适配器 |
|---|---|
| 无后端配置 | `LocalAdapter`（纯 IndexedDB） |
| 有 token + API URL | `HybridAdapter`（本地 + R2 异步备份） |

R2 备份失败只打 warning，不影响主流程。未配置 R2 时 Worker 静默降级，写操作返回成功。
...
| Key | 内容 |
|---|---|
| `echo-storage-v16` | 主状态（config、messages、characters 等） |
| `echo-saves` | 单角色存档列表 |
| `echo-multi-saves` | 双角色存档列表 |
| IndexedDB `image-store` | 角色头像、自定义背景等图片 |

**认证流程：**

1. 应用启动时检查 `localStorage` 是否有 token
2. 有 token → 初始化 HybridAdapter，进入应用
3. 无 token → 以 LocalAdapter 进入应用（或显示 GateScreen 输入密码）
4. 密码验证：POST `/api/auth` → 返回 token → 存入 `localStorage` → 切换为 HybridAdapter
...
源码位于 `echo-storage/cloudflare/`。R2 为**可选**，不配置时 Worker 正常运行，数据仅存本地。
...
# 取消 wrangler.toml 中 [[r2_buckets]] 的注释，填入 bucket_name
wrangler deploy
```
...
存档数据独立存储在 `echo-saves` / `echo-multi-saves` key 下，不混入主 store JSON，避免存档增多导致主状态读写变慢。
